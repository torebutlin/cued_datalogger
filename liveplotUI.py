...
class LiveplotApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.btn = QPushButton('Get Snapshot',btn_container)
        self.btn.resize(self.btn.sizeHint())
        self.btn.pressed.connect(self.get_snapshot)
    ...
    def update_line(self):
        ...
        weightage = np.exp(self.timedata / self.timedata[-1])
    ...
    def get_snapshot(self):
        if self.parent.data_tabs.currentWidget():
            data = self.rec.get_buffer()
            print(self.parent.data_tabs.currentWidget())
            self.parent.data_tabs.currentWidget()\
            .plotline.setData(x=self.timedata, y=data.reshape((len(data),)))
